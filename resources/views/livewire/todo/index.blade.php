    ...
    <div class="w-full p-4 md:max-w-2xl">
        ...
        <ul class="space-y-4">
            @foreach ($todoList as $todo)
                <li wire:key="list:{{ $todo->id }}" @class([
                    ...
                    <label for="toggle.{{ $todo->id }}" @class(['line-through' => $todo->done, 'w-full cursor-pointer'])>
                        {{ $todo->description }}
                    </label>

                    <div class="flex-table-hidden-actions flex justify-between gap-2">
                        @if (!$todo->done)
                            <x-button wire:click="edit('{{ $todo->id }}')" rounded flat primary icon="pencil" />
                        @endif
